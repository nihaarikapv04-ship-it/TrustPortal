...
interface PendingDefectRequest {
  payload: any;
  urlPath: string;
  timestamp: number;
}
...
async function handleProcessDefect(payload: any, urlPath: string): Promise<any> {
...
async function handleFeedback(patchId: string, feedback: string): Promise<void> {
  try {
...
function getAttributeForIssue(issueType: string): string {
  if (issueType === "img-alt") return "alt";
  return "aria-label";
}

function generateFallbackLabel(issueType: string, ctx: any): string {
