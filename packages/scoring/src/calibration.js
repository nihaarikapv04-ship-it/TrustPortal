export class UncalibratedModel {
    status = "uncalibrated";
    calibrate(rawConfidence) {
        return Math.max(0.0, Math.min(1.0, rawConfidence));
    }
}
export class PlattScalingModel {
    status = "fitted";
    a;
    b;
    constructor(a = -1.2, b = 0.5) {
        this.a = a;
        this.b = b;
    }
    calibrate(rawConfidence) {
 ...
 */
export function computeECE(predictions, labels, numBins = 10) {
