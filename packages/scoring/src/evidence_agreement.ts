...
export class EvidenceAgreementCalculator {
   ...
   */
  public computeAgreement(proposedLabel: string, context: SafeContext): number {
    if (!proposedLabel || !proposedLabel.trim()) return 0.0;

    const labelTokens = this.tokenize(proposedLabel);
    if (labelTokens.length === 0) return 0.0;
    ...
    if (evidenceSources.length === 0) return 0.20; // Default baseline baseline
