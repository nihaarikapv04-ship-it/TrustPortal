import { SafeContext, Decision } from "@trustportal/schemas";
import { TrustDecision, SignalComponents, AblationConfig, DEFAULT_ABLATION_CONFIG } from "./types.js";
...
export class TSIFRiskGate {
  ...
  private ablationConfig: AblationConfig;

  constructor(
    calibrationModel: CalibrationModel = new UncalibratedModel(),
    ablationConfig: AblationConfig = DEFAULT_ABLATION_CONFIG
  ) {
  ...
  public evaluateProposal(
    ...
    const signals: SignalComponents = signalExtractor.extractSignals(
      proposedLabel,
      rawModelConfidence,
      context,
