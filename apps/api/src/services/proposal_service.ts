import { ProposalRequest, ProposalResponse, SemanticPatch } from "@trustportal/schemas";
...
export class ProposalService {
   ...
   */
  public async processProposal(request: ProposalRequest): Promise<ProposalResponse> {
    ...
    const trustDecision = tsifRiskGate.evaluateProposal(
      proposal.label,
      proposal.modelConfidence,
      firewallResult.safeContext
    );

    // 5. Construct Authorized Patch if Decision is "auto"
    let patch: SemanticPatch | undefined = undefined;
