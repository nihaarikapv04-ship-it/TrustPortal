...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    if (originText)
        originText.textContent = currentOrigin;
    async function updateUI() {
        ...
        if (issuesCount)
            issuesCount.textContent = String(status.issuesDetected);
        if (proposalsCount)
            proposalsCount.textContent = String(status.proposalsPending);
        if (repairsCount)
            repairsCount.textContent = String(status.repairsApplied);
        if (toggleBtn) {
            if (status.enabled) {
