...
(() => {
  ...
  // src/messaging_types.js
  var ExtensionMessageSchema = external_exports.object({
    type: external_exports.string(),
    payload: external_exports.any().optional(),
    requestId: external_exports.string().optional()
  });
  var SetSiteEnabledPayloadSchema = external_exports.object({
  ...
  var contentMessaging = new ContentMessaging();

  // src/content/index.ts
  var TrustPortalClientRuntime = class {
    initialized = false;
    ...
    async init() {
      ...
      console.log(`\u{1F6E1}\uFE0F TrustPortal Runtime Initializing on [${this.currentOrigin}]...`);
    ...
    scanAndRemediate() {
      const interactiveElements = document.querySelectorAll("button, a, input, [role='button'], svg");
      ...
      console.log(`\u{1F6E1}\uFE0F TrustPortal remediated ${this.repairedElements.length} accessibility defects on [${this.currentOrigin}]`);
    }
    renderFloatingBadge() {
    ...
    `;
      ...
      badge.innerHTML = `
      \u{1F6E1}\uFE0F <strong>TrustPortal Active</strong> ${statusDot}
    ...
    toggleInspectorCard(root) {
      ...
      overflow-y: auto;
    `;
      const listHtml = this.repairedElements.length > 0 ? this.repairedElements.map((item) => `
        ...
        `).join("") : `<div style="color: #94a3b8; padding: 12px; text-align: center;">No accessibility defects detected on this page view.</div>`;
      card.innerHTML = `
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; border-bottom: 1px solid #334155; padding-bottom: 8px;">
        <strong style="font-size: 13px; color: #38bdf8;">\u{1F6E1}\uFE0F TrustPortal Live Inspector</strong>
    ...
    toggleVisualHighlights() {
      ...
      this.repairedElements.forEach((item) => {
        if (this.highlightsActive) {
          ...
          item.elementRef.setAttribute("title", `\u{1F6E1}\uFE0F TrustPortal Patched: aria-label="${item.remediatedLabel}"`);
        } else {
  ...
  var runtime = new TrustPortalClientRuntime();
