...
import { ConfirmationViewModel } from "../src/messaging_types.js";
...
describe("TrustPortal Confirmation UI & Workflow Tests", () => {
  ...
  test("Accept action invokes PatchApplicator and updates target DOM attribute", () => {
    ...
    // Simulate clicking Accept button inside Shadow DOM
    const host = document.querySelector("trustportal-host");
    const shadow = host?.shadowRoot;
    const acceptBtn = shadow?.querySelectorAll("button")[1]; // Accept button
    acceptBtn?.click();

    // Target element MUST now have aria-label applied safely!
